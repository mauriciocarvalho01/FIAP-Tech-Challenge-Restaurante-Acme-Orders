...
export class RegisterController {
  ...
  async getClient({
    cpf,
  }: Register.FindClientInput): Promise<
    HttpResponse<Register.FindClientOutput | Error>
  > {
  ...
  async createClient(
    clientData: Register.InsertClientInput
  ): Promise<HttpResponse<Register.InsertClientOutput | Error>> {
    if (!clientData.clientId)
      clientData.clientId = this.tokenHandler.generateUuid();

    const client = await this.registerRepo.insertClient(clientData);
    if (client === undefined)
      return badRequest(new Error('Cant insert client'));
