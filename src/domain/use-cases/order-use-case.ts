...
export class OrderManager implements OrderService {
  ...
  calculateOrderValues(
    orderData: OrderService.CalculateOrderValueInput[]
  ): OrderService.CalculateOrderValueInput[] {
    if (!orderData)
  ...
  calculateOrderValue(
    orderData: OrderService.GenericType
    ...
    const totalPrice =
      orderData.orderProducts.reduce(
        (acc: number, product: OrderService.GenericType) => {
      ...
      orderData.orderProducts
        .flatMap(
          (product: OrderService.GenericType) => product.ingredientProducts
        )
  ...
  validateOrderStatusRule(
    order: OrderService.GenericType,
    newStatus?: string
  ): boolean {
    // Status permitidos no pedido
    if (
      order.status &&
      !['Recebido', 'Em Preparação', 'Pronto', 'Finalizado'].includes(
        order.status
      )
    ) {
      return false;
    ...
    if (
      order.status &&
      !newStatus &&
      ['Pronto', 'Finalizado'].includes(order.status)
    ) {
      return false;
    }
  ...
  async saveOrder(
    ...
    if (!orderData.orderId)
      orderData.orderId = this.tokenHandler.generateUuid();
  ...
  async saveOrderProduct(
    ...
    const productOrder =
      await this.orderRepo.saveOrderProduct(productOrderData);
  ...
  async saveIngredientProduct(
    ...
    const ingredientProduct = await this.orderRepo.saveIngredientProduct(
