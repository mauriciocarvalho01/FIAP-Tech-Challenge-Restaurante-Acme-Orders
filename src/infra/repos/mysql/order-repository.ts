...
export class OrderRepository extends MySQLRepository implements Order {
            ...
            .map((order) => ({
              ...
              orderProducts: order.orderProducts.map(
                (op: Order.GenericType) => ({
                  ...
                  ingredientProducts: op.ingredientProducts?.map(
                    (ip: Order.GenericType) => ({
                      id: ip.id,
  ...
  async findOrder({
    orderId,
  }: Order.FindOrderInput): Promise<Order.FindOrderOutput> {
    try {
      const orderRepo = this.getRepository(this.orderEntity);
  ...
  async saveOrderProduct(
    orderProductData: Order.InsertOrderProductInput
    ...
    try {
      ...
      const orderProduct = await orderProductRepo.save({
  ...
  async saveIngredientProduct(
    ingredientProductData: Order.InsertIngredientProductInput
  ): Promise<Order.InsertIngredientProductOutput> {
    try {
      const ingredientProductRepo = this.getRepository(
        this.ingredientProductEntity
      );
      ...
      if (!existingIngredientProduct) {
        const ingredientProduct = await ingredientProductRepo.insert(
      ...
      const ingredientProduct = await ingredientProductRepo.save({
  ...
  async findProduct({
    productId,
  }: Order.FindProductInput): Promise<Order.FindProductOutput> {
    try {
  ...
  async findIngredient({
    ingredientId,
  }: Order.FindIngredientInput): Promise<Order.FindIngredientOutput> {
    try {
  ...
  async deleteOrder(
    orderData: Order.FindOrderInput
  ): Promise<Order.DeleteOrderOutput> {
    try {
      const orderRepo = this.getRepository(this.orderEntity);
  ...
  async deleteOrderProduct(
    orderProductData: Partial<Order.InsertOrderProductInput>
  ): Promise<Order.DeleteOrderProductOutput> {
    try {
      const orderProductRepo = this.getRepository(this.orderProductEntity);
  ...
  async deleteIngredientProduct(
    ingredientProductData: Partial<Order.InsertIngredientProductInput>
  ): Promise<Order.DeleteIngredientProductOutput> {
    try {
      const ingredientProductRepo = this.getRepository(
        this.ingredientProductEntity
      );
